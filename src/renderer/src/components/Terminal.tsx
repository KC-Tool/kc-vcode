...
const darkTheme = {
  ...
  black: '#333', red: '#f44747', green: '#6a9955', yellow: '#cca700',
...
const lightTheme = {
  ...
  black: '#333', red: '#d32f2f', green: '#388a34', yellow: '#bf8803',
  blue: '#0066cc', magenta: '#7b1fa2', cyan: '#00897b', white: '#1e1e1e',
...
export default function TerminalPanel({ cwd, visible, theme }: TerminalProps) {
  ...
  // cd to workspace when cwd changes
  useEffect(() => {
    if (cwd && termRef.current) {
      window.electronAPI.terminalInput(`cd "${cwd}"\n`)
    }
  }, [cwd])
